...
public:
  ...
  */
  ...
  consumePendingFrame(Minicap::Frame* frame) {
    ...
    result = screenshot.update(mDisplay, android::Rect(), 240, 320, 0, -1U, false, 0);
    MCINFO("ScreenshotClient::update() result %s", error_name(result));
	...
	MCINFO("Nokia Device base %s",base);
	MCINFO("Nokia Device width %d",width);
	MCINFO("Nokia Device height %d",height);
	MCINFO("Nokia Device stride %d",stride);
	
    uint16_t* src = (uint16_t*) base;
    
    MCINFO("Nokia Device src %d",src);
